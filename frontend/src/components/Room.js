...
class Room extends Component {
        ...
        {this.state.seatmap != null ? (
          <div>
                ...
                if (!seat.isReserved) {
                  return (
                    <div
                      key={seat.label}
                      className={
                        seat.isSelected ? "selectedSeat" : "defaultSeat"
                      }
                      onClick={() => this.handleSeatPick(seat.index)}
                    >
                      {seat.label}
                    </div>
                  );
                  ...
                  return (
                    <div key={seat.label} className={"reservedSeat"}>
                      {seat.label}
                    </div>
                  );
                }
            ...
            <div className="checkout">
              ...
              <div className="buy-btn" onClick={this.makePurchase}>
                Purchase tickets
              </div>
            </div>
            {/* 
  ...
  getRoomInfo = () => {
    if (this.state.nowPlaying) {
      axios
        .get(
          `${urls.cinemaUrls.cinema}/${this.state.nowPlaying.cinemaId}/rooms/${this.state.nowPlaying.roomId}`
        )
        .then((res) => {
  ...
  checkCouponCode = (event) => {
    ...
    if (this.state.codeInput) {
      ...
      client
        .get(`${urls.invoiceUrls.getCoupon}${this.state.codeInput}`)
        .then((res) => {
          if (res.status === 200) {
            this.setState({
              correctCode: true,
              correctCodeId: res.data.data.list[0].id,
            });
            console.log("SUCCESS!!!");
          }
        })
